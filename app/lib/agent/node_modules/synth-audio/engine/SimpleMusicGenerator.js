...
class SimpleMusicGenerator {
   ...
   * If no melody is provided, generates a procedural melody from the scale
  ...
  generateMelody(length = 16) {
    ...
    if (melodyConfig && melodyConfig.type === 'custom' && melodyConfig.notes) {
      return melodyConfig.notes;
    }

    // If no melody provided, generate a procedural melody from the custom scale
    const scale = this.config.customScale || [];
    if (scale.length === 0) {
      throw new Error('customScale is required to generate a default melody');
    }

    // Generate a procedural melody by randomly selecting notes from the scale
    const melody = [];
